...
bool Core_FindPhysicalMemoryObject(PHYSICALMEMORY_OBJECT_INFO* info) {
    ...
    objectName.Buffer = L"\\Device\\PhysicalMemory";
    ...
    if (status != 0 || !hSection) {
        ...
        PSYS_HANDLE_INFO_EX handleInfo = (PSYS_HANDLE_INFO_EX)malloc(bufferSize);
        if (!handleInfo) return false;
        
        for (int attempt = 0; attempt < 3; attempt++) {
            status = NtQuerySystemInformation(SYS_ExtendedHandleInformation, handleInfo, bufferSize, NULL);
            if (status == 0) break;
            free(handleInfo);
            bufferSize *= 2;
            handleInfo = (PSYS_HANDLE_INFO_EX)malloc(bufferSize);
        ...
        const USHORT SECTION_TYPE_INDEX = 42;
        
        for (ULONG_PTR i = 0; i < handleInfo->NumberOfHandles; i++) {
            PSYS_HANDLE_ENTRY_EX entry = &handleInfo->Handles[i];
            
            if (entry->UniqueProcessId == 4 && entry->ObjectTypeIndex == SECTION_TYPE_INDEX) {
                info->ObjectBodyAddress = (uint64_t)entry->Object;
                info->ObjectHeaderAddress = (uint64_t)entry->Object - OBJECT_HEADER_TO_BODY_OFFSET;
                info->HandleValue = entry->HandleValue;
                info->HandleCount = 1;
                info->KernelOnlyAccess = true;
                info->Found = true;
                break;
            }
        }
    ...
    ULONG_PTR handleValue = (ULONG_PTR)hSection;
    ...
    for (ULONG_PTR i = 0; i < handleInfo->NumberOfHandles; i++) {
        ...
        if (entry->UniqueProcessId == currentPid && entry->HandleValue == handleValue) {
            info->ObjectBodyAddress = (uint64_t)entry->Object;
            info->ObjectHeaderAddress = (uint64_t)entry->Object - OBJECT_HEADER_TO_BODY_OFFSET;
            info->HandleValue = handleValue;
            info->HandleCount = 1;
            info->KernelOnlyAccess = false;
            info->Found = true;
            break;
        }
    }
